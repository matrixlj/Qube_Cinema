...
public class RegistryMonitor : IDisposable
{
	...
	private readonly ManualResetEvent _eventTerminate = new ManualResetEvent(initialState: false);
	...
	protected virtual void _OnRegChanged()
	{
	...
	private void InitRegistryKey(string name)
	{
		...
		default:
			...
			throw new ArgumentException("The registry hive '" + array[0] + "' is not supported", "name");
		}
	...
	private void ThreadLoop()
	{
		...
		try
		{
			...
			{
				num = RegNotifyChangeKeyValue(phkResult, bWatchSubtree: true, _regFilter, autoResetEvent.Handle, fAsynchronous: true);
				if (num != 0)
				{
					throw new Win32Exception(num);
				}
